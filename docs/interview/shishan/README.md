...
- volatile 主要作用是保证**可见性**以及**有序性**。
- 有序性涉及到**指令重排**、**内存屏障**等概念。
- volatile 不能保证原子性，原子性的问题还是需要依赖 synchronized、ReentrantLock 等加锁机制来解决。
...
- 分布式系统为了保证**数据传输**的可靠性，会采用 `Quorum` 机制，对数据写入结果进行判定。
- `Quorum` 机制的核心思想是需要保证**指定时间内**集群内**大多数**机器响应成功才认定为数据写入成功。
- 使用 `Quorum` 机制的系统一般要控制在奇数的数量，便于统计判定响应结果。
- `Quorum` 机制实际上在很多分布式系统、中间件系统中都有非常广泛的使用。
- **简单的实现就是异步开启线程发送数据给下游系统所有的机器，同时进入一个 while 循环等待下游系统的 `Quorum` 数量的机器返回响应结果。**
...
| 消息可靠性              | 有较低的概率丢失数据。                  | 基本不丢失。                                         | 经过参数优化配置，可以做到 0 丢失。                          | 同 RocketMQ。                                                |
...
- 每条写到 MQ 的数据都被多副本冗余了，也就是写的每一条消息都被复制到了其他的机器上去了。
...
- Kafka 通过多副本冗余 Partition 分片的机制保证数据的不丢失。
- 多副本冗余几乎是现在任何一个优秀的分布式系统都一般要具备的功能。
- ISR全称是 `In-Sync Replicas`，也就是保持同步的副本，他的含义就是，跟 Leader 始终保持同步的 Follower 信息。
- acks 参数，是在 Kafka Producer，也就是生产者客户端里设置，acks 三种常见的值可以设置，分别是：0、1 和 all。
  - `0` 只要把消息发送出去，即使消息没有刷盘到 Kafka Leader，也算成功。
  - **`1` 只要 Partition Leader 接收到消息而且写入本地磁盘，就认为成功，不管他其他的 Follower 有没有同步过去这条消息。是 Kafka 的默认设置。**
  - `all` Partition Leader 接收到消息之后，还必须要求 ISR 列表里跟 Leader 保持同步的那些 Follower 都要把消息同步过去，才能认为这条消息是写入成功。
  - **`acks=all` 必须跟 ISR 列表里至少有 2 个以上的副本配合使用，如果 Partition 只有一个副本，该配置是没有意义的。**
...
- `CopyOnWrite` 写时拷贝，主要的思想是取消加锁，取消互斥带来的性能降低，在发生写入操作的时候，**利用拷贝的副本来执行**。
- `CopyOnWrite` 是一种以空间换时间的思想。
- 当写入操作完成后，通过 `volatile` 关键字让其他线程可以感知到数据变化，其他线程可以读到这个变量引用的最近的值。

#### CopyOnWriteArrayList

- JDK 中 `CopyOnWriteArrayList` 提供了一种写时复制方案的实现思路。
- 写入的方法是需要加锁的，保证写入操作的互斥性，避免数据错乱。
- 基于 `volatile` 的特性，读线程可以感知到最新的数据。

#### Kafka中的应用

- 在 Kafka 源码中，客户端在向 Kafka 写数据的时候会通过合并 `batch` 和 `request` 的方式提升吞吐量。
- 内存缓冲中使用的数据结构就是 Kafka 自己实现的 `CopyOnWriteMap`。
- Kafka 这个核心数据结构在这里之所以采用 CopyOnWriteMap 思想来实现，就是因为这个 Map 的 key-value 对，其实没那么频繁更新。
  - `key` TopicPartition。
  - `value` Deque。
- 采用了 CopyOnWrite 思想来实现的 Map，**避免更新 key-value 的时候阻塞住高频的读操作，实现无锁的效果，优化线程并发的性能。**

------

### Kafka的GC优化

- Kafka 客户端会采用 batch 的方式批量发送消息提升吞吐量，在发送 batch 之后，需要释放内存。
- 如果等待 JVM GC 进行内存释放，由于本身 Kafka 吞吐量很大，可能会导致频繁的 GC，影响系统的性能。
- Kafka 采用的在内存中开启指定大小的缓存区间的方式，使用过程申请空间，batch发送后清空空间并归还缓冲区，通过这样的方式循环利用。
- Kafka 内存缓冲机制解决了 JVM GC 可能带来的恶劣影响，优化的系统性能。
...
- 重量级锁是 Java 虚拟机中最为基础的锁实现。
- 重量级锁的实现中，Java 虚拟机会阻塞加锁失败的线程，并且在目标锁被释放的时候，唤醒这些线程。
- 在 Linux 中，这是通过 pthread 库的互斥锁来实现的。
- 这些操作将涉及系统调用，需要从操作系统的用户态切换至内核态，其开销非常之大。
- 为了尽量避免昂贵的线程阻塞、唤醒操作，Java 虚拟机会在线程进入阻塞状态之前，以及被唤醒后竞争不到锁的情况下，进入**自旋状态**，在处理器上空跑并且轮询锁是否被释放。

#### 自旋锁

- 自旋锁属于 JVM 层面的。

- 自旋很耗费资源，所以可以根据以往自旋等待时是否能够获得锁，来动态调整自旋的时间。
- 处于阻塞状态的线程，并没有办法立刻竞争被释放的锁。然而，处于自旋状态的线程，则很有可能优先获得这把锁。所以 **Synchronized 不是公平的**。

#### 轻量级锁

- Java 虚拟机采用了轻量级锁，来避免重量级锁的阻塞以及唤醒。

#### 对象头锁标识

- 对象头中有一个标记字段，它的最后两位便被用来表示该对象的锁状态。
  - `00` 代表轻量级锁。
  - `01` 代表无锁（或偏向锁）。
  - `10` 代表重量级锁。
  - `11` 则跟垃圾回收算法的标记有关。
...
- 偏向锁针对的是从始至终只有一个线程请求某一把锁。
- 偏向锁是轻量级锁的更进一步的乐观情况。
- 当设置偏向锁时，Java 虚拟机需要将该 epoch 值复制到锁对象的标记字段中。

#### 锁升级过程

- 当进行加锁操作时，Java 虚拟机会判断是否已经是重量级锁。
  - 如果不是，它会在当前线程的当前栈桢中划出一块空间，作为该锁的锁记录，并且将锁对象的标记字段复制到该锁记录中。
  - Java 虚拟机会尝试用 CAS 操作替换锁对象的标记字段。
- 发生锁竞争的时候。
  - 如果是**该线程重复获取同一把锁**。此时，Java 虚拟机会将 0 加入锁记录，以代表该锁被重复获取。
  - **如果是其他线程持有该锁**。此时，Java 虚拟机会将这把锁膨胀为重量级锁，并且阻塞当前线程。
...
- Hadoop 是目前大数据领域最主流的一套技术体系，包含了多种技术。
  - `HDFS` 分布式文件系统。
  - `YARN` 分布式资源调度系统。
  - `MapReduce` 分布式计算系统。
...
- `DataNode`
  ...
  - 一个集群内会有多个 DataNode。
- `NameNode`
  - NameNode 是**负责管理整个HDFS集群**的节点。
  - 存储了 HDFS 集群的所有元数据，如文件目录树、权限的设置、副本数的设置。

#### 分布式计算

- 写一个大SQL，人家会拆分为很多的计算任务，放到各个机器上去，每个计算任务就负责计算一小部分数据。

- 通过 `Hive` 写几百行的大SQL，底层基于 `MapReduce`。
- 用 `Spark` 写几百行的大SQL，底层是 `Spark Core` 引擎。

#### NameNode架构原理

- `文件目录树` 记录了分布式文件系统中文件的目录情况。
- 基于 HDFS 系统特性，目录树会被频繁的读写，目录树直接存储在 `NameNode` 的内存中，宕机后可能有丢失的风险。
- HDFS 通过使用 `edits log` 的方式，在元数据修改的操作日志到磁盘文件里，不修改磁盘文件内容，就是顺序追加，以较高的性能记录文件目录树的变化。
- `NameNode` 重启后，通过重新读取 `edits log` 的方式回复元数据。

#### 节点种类

- `Active NameNode` 主节点。
- `Standby NameNode` 备用节点。
- `JournalNodes` 日志节点集群。
- `DataNode` 数据节点。

#### HDFS优化流程

1. 每次 Active NameNode（主节点）修改一次元数据都会生成一条 edits log，**除了写入本地磁盘文件，还会写入 JournalNodes 集群。**
2. Standby NameNode 会从 JournalNodes 集群拉取 edits log，应用到自己内存的文件目录树里，跟 Active NameNode 保持一致。
3. 每隔一段时间，Standby NameNode 都把自己内存里的文件目录树写一份到磁盘上的 fsimage，这可不是日志，这是完整的一份元数据。**这个操作就是所谓的 `checkpoint` 检查点操作。**
4. fsimage 上传到到 Active NameNode，接着清空掉 Active NameNode 的旧的 edits log 文件。
5. `Active NameNode` 重启把Standby NameNode 传过来的 fsimage 直接读到内存里，**这个 fsimage 直接就是元数据**，不需要做任何额外操作，纯读取，效率很高。
6. 在加载少量不在 fsimage 中的的 edits log 即可。

#### 高可用保证

- `Active NameNode` 主节点和 `Standby NameNode` 备节点在内存中存储的文件目录树是一样的，所以主节点发生故障后，可以直接切换备节点，完成主备切换，继续对外提供可用性。
- 上传的文件会被拆分成多个 block 存储在集群内的多个 DataNode 内，避免某个 DataNode 宕机导致的数据丢失，默认会为每个 block **创建三个副本并保存在不同的机器上**。
- 每个 `block` 的大小默认是 128MB。
...
- 当面对每秒上千次的请求时，HDFS 的 NameNode 每次请求都需要修改一条元数据，编写一条 edits log。
  - 写入本地磁盘。
  - 通过网络传输给 JournalNodes 集群。
- edits log 中的全局顺序递增的 transactionId `txid` 的生成需要是串行的，来确保 txid 是递增的。

#### 解决方案

- 采用内存缓冲的方式，将生成 txid + 写入磁盘任务组合，提高吞吐量。
- 为了保证可以不并发读写一块共享内存区域带来的性能问题，HDFS 采用了 **double-buffer 双缓冲机制**，将内存缓冲分成两个部分。
  - 其中一个部分可以写入。
  - 另外一个部分用于读取后写入磁盘和JournalNodes。
- **分段加锁机制 + 内存双缓冲机制**
  - 将写入缓存区任务和传输给 JournalNodes 任务分开执行，需要不同的锁。
- **缓冲数据批量刷磁盘 + 网络的优化**
  - 大量的线程并发过来的话，都会在这里快速的第二次获取锁，然后发现有人在写磁盘和网络，快速的释放锁，休眠。
  - 如果第二把锁的幸运儿线程的 txid 是排在自己之后的，那么肯定就把自己的 edits log 从缓冲里写入磁盘和网络了，当前线程可以直接结束自己的任务，不用休眠等待。
  - 幸运儿线程在完成写入之后，交换两个缓冲区。

#### 图示

<img src="README-image/HadoopNameNode%20%E9%AB%98%E5%B9%B6%E5%8F%91%E5%8E%9F%E7%90%86.jpg" alt="HadoopNameNode 高并发原理" style="zoom: 33%;" />

#### 总结

- 通过上述那套复杂的机制，尽最大的努力保证，一个线程可以批量的将一个缓冲中的多条 edits log 刷入磁盘和网络。
- 其他的线程可以快速的高并发写入 edits log 到内存缓冲里，不会阻塞其他的线程写 edits log。
...
1. 分割文件为 128MB 每个的 block。
2. HDFS 客户端把一个一个的 block 上传到第一个 DataNode。
3. 第一个 DataNode 会把这个 block 复制一份，做一个副本发送给第二个 DataNode。
4. 第二个 DataNode 发送一个 block 副本到第三个 DataNode。

#### Chunk缓冲机制

- `chunk` 是一个 512 字节大小的数据片段。
- 缓冲数组可以容纳多个 chunk 大小的数据在里面缓冲。

####  Packet数据包机制

- 当 chunk 缓冲数组都写满了之后，就会把这个 chunk 缓冲数组进行一下 **chunk 切割**，切割为一个一个的 chunk，一个 chunk 是一个数据片段。
- 多个 chunk 会直接一次性写入另外一个内存缓冲数据结构，就是 **Packet 数据包**。
- 一个 Packet 数据包，设计为可以容纳 127 个 chunk，大小大致为 64 mb。
- 大量的 chunk 会不断的写入 Packet 数据包的内存缓冲中。
- 通过 Packet 数据包机制的设计，可以在内存中容纳大量的数据，**进一步避免了频繁的网络传输影响性能。**

####  内存队列异步发送机制

- **当一个 Packet 被塞满了 chunk 之后，就会将这个 Packet 放入一个内存队列来进行排队。**
- `DataStreamer` 线程会不断的获取队列中的 Packet 数据包，通过网络传输直接写一个 Packet 数据包给 DataNode。
- 如果一个 Block 默认是 128mb 的话，那么一个 Block 默认会对应两个 Packet 数据包，每个 Packet 数据包是 64MB。
- **传送两个 Packet 数据包给 DataNode 之后，就会发一个通知说，一个 Block 的数据都传输完毕。**

#### 图示

<img src="README-image/HadoopNameNode%20%E9%AB%98%E5%B9%B6%E5%8F%91%E5%8E%9F%E7%90%86-6615809.jpg" alt="HadoopNameNode 高并发原理" style="zoom:36%;" />
...
- HDFS 上的文件是不可以进行并发写入的。
- **文件契约机制**是指同一时间只能有一个客户端获取 NameNode 上面一个文件的契约，然后才可以写入数据。此时如果其他客户端尝试获取文件契约的时候，会就获取不到，等待当前占有契约的客户端释放。
- 在获取到了文件契约之后，在写文件的过程期间，那个客户端需要开启一个线程，不停的发送请求给 NameNode 进行文件**续约**，类似于 Redisson 中 RLock 的看门狗机制。

#### 性能瓶颈

- 大规模部署的情况下，存在大量客户端。
- NameNode 内部维护的文件契约列表非常大，监控契约的后台线程又需要频繁的检测契约是否过期，影响性能。

#### 优化方案

- 通过使用 `TreeSet` 来存储文件契约列表。
- 根据最近一次续约的时间进行排序，每次都把续约时间最老的契约排在最前头。
- TreeSet 是一种可排序的数据结构，他底层基于 TreeMap 来实现。
- TreeMap 底层则基于**红黑树**来实现，可以保证元素没有重复，同时还能按照我们自己定义的排序规则在你每次插入一个元素的时候来进行自定义的排序。
- **后台监控线程在每次检查的时候，只需要检查续约时间最久的那个契约即可。**
- 通过这个 TreeSet 排序+优先检查最旧契约的机制，有效的将大规模集群下的契约监控机制的性能提升至少 10 倍以上。
...
- 将采集的存储数据在自己的数据中心里。
- 通过实时的运算大量的几百行~上千行的 SQL 来生成各种数据报表。
- 提供这些数据报表给用户来分析。
...
- 通过自研的数据库 binlog 采集中间件采集业务系统数据库的日志同步数据。
- 数据平台里做一些定时调度任务，每隔几秒钟就运行上百个复杂大 SQL，计算各种报表的数据并将结果存储到 MySQL 库中。
- 用户只要对报表刷新一下，立马就可以从 MySQL 库里查到最新的报表数据。

#### 数据量激增的隐患

- 随着数据量激增，单表数据达到亿级。
- 一个大 SQL可能都要几个小时才能跑完。
- MySQL 的 cpu 负载压力直接 100%，发生 MySQL 数据库服务器宕机事件。

#### 离线计算与实时计算拆分

##### 离线计算

- 将昨天以及昨天以前的数据都放在大数据存储中，进行离线存储和离线计算。
- 每天凌晨，将业务系统 MySQL 库中的昨天以前的数据，作为离线数据导入 Hadoop HDFS 中进行离线存储。
- 凌晨就基于 Hive / Spark 对离线存储中的数据进行离线计算。

##### 实时计算

- 只有今天的数据是实时的采集的。
- 在每天零点的时候，从数据平台的存储中清理掉昨天的数据，仅仅保留当天一天的数据而已，大幅度降低了要放在 MySQL 中的数据量。

#### 持续增长的数据量和计算压力

- 单日数据量将会达到几亿，甚至十亿的级别。
- 单表数据量上亿，无论怎么优化 SQL 性能，都无法保证 100 多个几百行的复杂 SQL 可以快速的运行完毕。
- 实时计算链路面临是巨大的计算压力。
- 还有另外一个问题，那就是单个 MySQL 数据库服务器的存储容量是有限的。
- 如果一旦单日数据量达到甚至超过了单台 MySQL 数据库服务器的存储极限，那么此时也会导致单台 MySQL 数据库无法容纳所有的数据。

#### 大数据领域的实时计算技术的缺陷

- 当时项目背景下大数据领域的实时计算结果并不理想。

#### 分库分表解决数据扩容问题

- 将一个库拆分为多库，不用的库放在不同的数据库服务器上，同时每个库里放多张表。
- 每个数据库服务器放一部分的数据，而且随着数据量日益增长，可以水平扩展，做到按需扩容。
- 每个库里单表分为多表，控制单表的数据量在几百万的量级，性能优化到极致的 SQL 可以级出结果。

#### 读写分离降低数据库服务器的负载

- 分库分表之后，如果对每个数据库服务器又是写入又是读取的话，会导致数据库服务器的 CPU 负载和 IO 负载非常的高。
- 需要将 MySQL 做读写分离的部署。
- 每个主数据库服务器都挂了多个从数据库服务器。
- 写只能写入主库，查可以从从库来查。

#### 自研的滑动窗口动态计算引擎

对系统的整体响应时间进行进一步的优化，达到秒级的时效性。
...
- 数据平台中的核心计算引擎，不再是每隔几十秒就运行大量 SQL 对当天所有的数据全部计算一遍了，而是对一个接一个的滑动时间窗口，根据窗口标签提取出那个窗口内的数据进行计算，计算的仅仅是最近一个滑动时间窗口内的数据。
- 接着对这个滑动时间窗口内的数据，可能最多就千条左右吧，运行所有的复杂 SQL 计算出这个滑动时间窗口内的报表数据，然后**将这个窗口数据计算出的结果，与之前计算出来的其他窗口内的计算结果进行合并**，最后放入 MySQL 中的报表内。

#### 离线计算链路的性能优化

- 离线计算链路的性能问题主要是由于每次导入都是**历史全量数据**导致的。
- 因为从业务库同步数据时，每天都涉及到数据的更新操作，而 hadoop 里的数据是没法跟业务库那样来进行更新的，因此最开始都是每天导入全量历史数据，作为一个最新快照来进行全量计算。
- 优化思路是**全量计算转增量计算**。
- 每天数据在导入 hadoop 之后，都会针对数据的**业务时间戳来分析和提取出来每天变更过的增量数据，将这些增量数据放入独立的增量数据表中**。
- 需要根据具体的业务需求，自动分析数据计算的基础血缘关系，有可能增量数据需要与部分全量数据混合才能完成计算，此时可能会提取部分全量历史数据，合并完成计算。计算完成之后，将计算结果与历史计算结果进行合并。

#### 架构图

![图片](README-image/640.jpeg)
...
- 系统部署在两台机器上，但是同一时间只有一台机器是会运行的，但是另外一台机器是备用的。
- 处于 active 状态的系统会将滑动窗口计算引擎的计算状态和结果写入 zookeeper中，作为元数据存储起来。
- 关于元数据基于 zookeeper 来存储的方案，参考 Storm 流式计算引擎的架构方案。
- zookeeper 的读写性能非常的高，而且 zookeeper 集群自身就可以做到非常高的可用性，同时还提供了大量的分布式系统需要的功能支持，包括分布式锁、分布式协调、master选举、主备切换等等。
- 基于 zookeeper 实现了 active-standby 的主备自动切换，如果 active 节点宕机，那么 standby 节点感知到，会自动切花为 active，同时自动读取他们共享的一个计算引擎的中间状态，然后继续恢复之前的计算。

#### Master-Slave 架构的分布式计算系统

- 将数据平台系统彻底重构和设计为一套分布式的计算系统，**任务调度**与**任务计算**两个职责进行分离。
- 有一个专门的 Master 节点负责读取切分好的数据分片（也就是所谓的时间窗口，一个窗口就是一个数据分片）。
- Master 将各个数据分片的计算任务分发给多个 Slave 节点。
- Slave 节点的任务就是专门接收一个一个的计算任务，每个计算任务就是对一个数据分片执行一个几百行到上千行的复杂SQL语句来产出对应的数据分析结果。
- Master 节点为了避免其出现单点故障，沿用了之前的主备架构。
- Slave 节点主要负责计算任务，负载比较高，可以根据集群的计算负载情况进行水平扩容。

#### 弹性计算资源调度机制

- 由于系统的高峰和低谷的数据差异导致偶尔出现某台 Slave 机器积压了大量的计算任务的情况。
- 在 Master 节点中加入了计算任务 metrics 上报、计算任务耗时预估、任务执行状态监控、机器资源管理、弹性资源调度等机制。
- Master 节点会实时感知到各个机器的计算任务执行情况、排队负载压力、资源使用等情况。
- 根据计算任务的历史 metrics、预估当前计算任务的耗时、综合考虑当前各 Slave 机器的负载，来将任务分发给负载较低的 Slave 机器。
- 保证了线上Slave集群资源的均衡利用。

#### 分布式系统高容错机制

- 一旦将系统重构为分布式系统架构之后，就可能会出现各种各样的问题，此时就需要开发一整套的容错机制。
...
- 如果说某个计算任务在多个 Slave 中无法成功计算的话，此时会将这个计算任务储存在一个延时内存队列中，间隔一段时间过后，比如说等待高峰期过去，然后再重新尝试执行这个计算任务。
- 如果某个计算任务等待很长时间都没成功执行，可能是 hang 死了，那么 Master 节点会更新这个计算任务的版本号，然后分配计算任务给其他的 Slave 节点来执行。
- 使用版本号，计算结果 CAS 更新，避免出现数据覆盖的情况。

#### 架构图

![img](README-image/1J6IbIcPCLYAU5U6KO3GkJeibEVTQ9ayu7XZSXhBlcnE1V19iaNmxiaF1hodFXmhoWMlG7YTbTOWRx2P6a8fvGzZQ.jpeg)
...
- 目前系统重度依赖 MySQL，只有 MySQL 是可以支撑如此复杂的 SQL 语句完美运行。
- 为了系统的响应在秒级别的，目前集群里的 MySQL 数据库都部署在高配置的物理机上，32 核 + 128 G + SSD。
- 流量从亿级别到百亿，继续用当前架构的话，需要堆大量的高配置机器，成本过高。

#### 计算与存储分离的架构

- 目前架构里的致命问题之一，就是数据的存储和计算混在了一个地方，都在同一个MySQL库里。
- 数据直接写入一个存储，仅仅只是简单的写入即可。
- 在计算的时候从数据存储中提取你需要的那个数据分片里的可能就一两千条数据，写入另外一个专用于计算的临时表中，那个临时表内就这一两千条数据。
- 运行各种复杂SQL即可。
- 存储可以使用自研的 KV 存储或者 HBASE。
- 轻量级的分布式kv系统，一般设计理念都是支持一些简单的kv操作，大量的依托于内存缓存热数据来支持高并发的写入和读取。
- 因此在同等的机器资源条件下，kv存储对高并发的支撑能力至少是 MySQL 的数倍甚至数十倍。
- 每天的海量数据都存放在 KV 存储中，Slave计算引擎每次计算，都是根据那个数据分片从 kv 存储中提取对应的数据出来放入 MySQL 内的一个临时表，接着就是对那个临时表内的一两千条数据分片运行各种复杂SQL进行计算即可。

#### 自研纯内存SQL计算引擎

- 自研的SQL计算引擎来在纯内存里针对一个数据分片执行各种复杂的SQL。
- 基本上纯内存的SQL执行，都是毫秒级的，基本上一个时间分片的运算全部降低到毫秒级。

- 彻底消除了对 MySQL 的依赖，理论上，无论多大的流量过来，都可以通过立马扩容 kv 集群以及扩容 Slave 计算集群来解决。
- **分布式系统的本质就是尽量利用大量的廉价普通机器就可以完成高效的存储和计算。**

#### MQ削峰以及流量控制

- 对于高并发的非实时响应的写入请求，完全可以使用 MQ 中间件先抗住海量的请求，接着做一个中间的流量分发系统，将流量异步转发到 kv 存储中去，同时这个流量分发系统可以对高并发流量进行控制。
- 如果瞬时高并发的写入真的导致后台系统压力过大，那么就可以由流量分发系统自动根据设定的阈值进行流量控制，避免高并发的压力打垮后台系统。
- 在流控系统中，还做了很多的细节性的优化，比如说数据校验、过滤无效数据、切分数据分片、数据同步的幂等机制、100%保证数据落地到kv集群的机制保障，等等。

#### 数据的动静分离架构

- 如果SQL要对一些表进行关联计算，里面涉及到了一些大部分时候静态不变的数据，那些表的数据一般很少改变，因此没必要每次都走网络请求从kv存储里提取那部分数据。
- 可以在 Slave 节点对这种静态数据做个轻量级的 cache，然后只有数据分片里对应的动态改变的数据才从 kv 存储来提取数据。
- 通过这个数据的动静分离架构，基本上把 Slave 节点对 kv 集群的网络请求降低到了最少，性能提升到了最高。

#### 架构图

![img](README-image/1J6IbIcPCLZ2Q0QUMv7gaydns5wtUQtEKp1jsWD4Oh5H6V2Xpiaa7JKhvJmC3l8AuXQs33unngBOMiaKOfticWV0g.jpeg)
...
- 系统中用于存储计算后统计数据的 MySQL 数据库面临着过大的数据压力。
- 单台 MySQL 服务器的磁盘存储空间很快就要接近满掉，而且单表数据量都是几亿、甚至十亿的级别。
- 用户查询数据分析报告的时候，体验较差。

#### 分库分表 + 读写分离

- 分库分表 + 读写分离，是基于MySQL的架构中常见的优化方案，毕竟实施起来难度不是特别的高，而且速度较快，效果比较显著。
- 每台主库可以承载部分写入压力，单库的写并发会降低。
- 单个主库的磁盘空间可以降低负载的数据量，不至于很快就被打满。
- **单个数据表的数据量可以降低到百万级别，这个是支撑海量数据以及保证高性能的最佳实践，基本两三百万的单表数据量级还是合理的。**

####  数据的冷热分离架构

- 系统的实际使用过程中，大多查询都是来自于 js 脚本自动触发的实时数据查询，只有 1% 是查询历史区间数据。
- 针对该提到的问题，很明显要做的一个架构重构就是**冷热数据分离。**
- 将今日实时计算出来的热数据放在一个 MySQL 集群里。
- 将离线计算出来的冷数据放在另外一个 MySQL 集群里。
- 开发一个数据查询平台，封装底层的多个 MySQL 集群，根据查询条件动态路由到热数据存储或者是冷数据存储。

#### 自研Elasticsearch+HBase+纯内存的查询引擎

- 冷数据存储量日益增加，对应的 MySQL 集群需要不断的扩容，为了 1% 的查询请求不断的增加高配置服务器是不合适的。
- 大量分库分表后，MySQL 大量的库和表维护起来是相当麻烦的。
- 对冷数据的查询，一般都是针对大量数据的查询，比如用户会选择过去几个月，甚至一年的数据进行分析查询，此时如果纯用MySQL还是挺灾难性的。
- 冷数据 MySQL 集群的升级方案是**自研一套基于NoSQL来存储，然后基于NoSQL+内存的SQL计算引擎。**
- 将冷数据全部采用 ES+HBase 来进行存储，ES 中主要存放要对冷数据进行筛选的各种条件索引，比如日期以及各种维度的数据，然后 HBase 中会存放全量的数据字段。
- 因为 ES 和 HBase 的原生 SQL 支持都不太好，因此自研另外一套SQL引擎，专门支持这种特定的场景，就是基本没有多表关联，就是对单个数据集进行查询和分析，然后支持 NoSQL 存储+内存计算。
- 如果要做到对冷数据全部是单表类的数据集查询，必须要在冷数据进入 NoSQL 存储的时候，全部基于 ES 和 HBase 的特性做到多表入库关联，进数据存储就全部做成大宽表的状态，将数据关联全部上推到入库时完成，而不是在查询时进行。
- 对冷数据的查询，自研的 SQL 引擎首先会根据各种 where 条件先走 ES 的分布式高性能索引查询，ES 可以针对海量数据高性能的检索出来需要的那部分数据，这个过程用 ES 做是最合适的。
- 接着就是将检索出来的数据对应的完整的各个数据字段，从 HBase 里提取出来，拼接成完成的数据。
- 然后就是将这份数据集放在内存里，进行复杂的函数计算、分组聚合以及排序等操作。
- 全部基于自研的针对这个场景的查询引擎完成，底层基于 ElasticSearch、HBase、纯内存来实现。

#### 实时数据存储引入缓存集群

- 每个商家的实时数据其实不是频繁的变更的，在一段时间内，可能压根儿没变化，因此不需要高并发请求。
- 每秒 10 万级别的请求不应该都落到数据库上，不然可能需要不断的扩展读库。
- 引入了一个缓存集群，实时数据每次更新后写入的时候，都是写数据库集群同时还写缓存集群的，是双写的方式。
- 查询的时候是优先从缓存集群来走，此时基本上 90% 以上的高并发查询都走缓存集群了，然后只有 10% 的查询会落地到数据库集群。

#### 架构图

![图片](README-image/640-20220311172539929.jpeg)
